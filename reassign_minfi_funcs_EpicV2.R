# This script modifies tow minfi funcs to work with EPICv2

# reassign .isEPIC to detect EPICV2
is_epicv2 <- function(object) {
    annotation(object)["array"] == "IlluminaHumanMethylationEPICv2"
    }

assignInNamespace(".isEPIC", is_epicv2, ns = "minfi")


# reassign combineArrays to detect Epic V2
combineArraysv2 <- minfi::combineArrays
body(combineArraysv2)[[1]] <- quote({
...
.convertArray_450k_epic <- function(rgSet,
    ...
    outType <- match.arg(outType)
    .isRGOrStop(rgSet)
    stopifnot(.is450k(rgSet) || .isEPICv2(rgSet))
...
assignInNamespace("combineArrays", combineArraysv2, ns = "minfi")