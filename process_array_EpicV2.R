...
if (!length(args)==2){
	stop('Usage: Rscript process_array.R {idat directory} {output csv path}')
}
...
###### replace minifi functions
source("./reassign_minfi_funcs_EpicV2.R")
...
process_array <- function(array_dir, out_path, ref_sample_path=args[3])
{
	...
	# Read in Sample Sheet
    	targets <- read.metharray.sheet(array_dir)
    	rgSet <- read.metharray.exp(targets = targets,recursive = TRUE, verbose = TRUE, force=TRUE, extended=T)
	...
	#rgSet <- combineArrays(rgSet, ref.sample)
